...
O sistema foi estruturado em três componentes principais: relógio, aplicativo móvel e \textit{backend}. O relógio atua como fonte de frequência cardíaca, enviada ao aplicativo por mensagens \textit{Wear OS} \cite{androidWearMessageClient}. O aplicativo móvel é responsável pela autenticação do usuário, seleção da horda, coleta de localização e aceleração, recepção do BPM (batimentos por minuto), envio periódico da telemetria e renderização da corrida gamificada. O \textit{backend} mantém a sessão online como fonte de verdade, calcula o estado da corrida, atualiza o \textit{ranking} e persiste os dados duráveis.
